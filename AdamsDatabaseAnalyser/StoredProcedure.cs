...
namespace AdamsDatabaseAnalyser
{
    public class StoredProcedure
    {
        ...
        public Dictionary<string, Field> aliasFields = new Dictionary<string, Field>();




        //Used to read in a stored procedure from a file
        public StoredProcedure(FileInfo file)
        {
            ...
            for (int tokenIndex = 0; tokenIndex < tokens.Count; tokenIndex++)
            {
                ...
                if (thisTokenValue == "CREATE" && nextTokenValue == "PROCEDURE")
                {
                    //the next few tokens should hold the table name
                    ProcedureName = Utils.StripStringOfSquareBrackets(Utils.GetTokenValue(tokenIndex + 4, tokens));
                }
                #endregion


                #region Defining All Temp tables   
                if (ProcedureName != "" && thisTokenValue.ToLower() == "create" && nextTokenValue.ToLower() == "table")
                {
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower() == "select")
                {
                ...
                if (ProcedureName != "" && thisTokenValue == ProcedureName && Parameters.Count == 0)
                {
                    //This is where a function should start defining it's parameters
                    List<string> finisherList = new List<string>();
                    finisherList.Add("as");
                    finisherList.Add("with");
                    List<string> finisherNextList = new List<string>();
                    finisherNextList.Add("begin");
                    finisherNextList.Add("insert");
                    finisherNextList.Add("update");
                    finisherNextList.Add("execute");
                    finisherNextList.Add("select");
                    finisherNextList.Add("delete");
                    finisherNextList.Add("set");
                    finisherNextList.Add("declare");
                    finisherNextList.Add("if");
                    List<ParsedToken> parameterTokens = Utils.CreateTokenBlockUntilString(tokens, tokenIndex + 1, finisherList, finisherNextList);

                    //parameterTokens = Utils.CleanupParsedTokenList(parameterTokens, parameterTokens.Count - 1);
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower() == "declare")
                {
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower() == "from")
                {
                    ...
                    else
                    {
                        ...
                        if (targetView != null)
                        {
                            ...
                            foreach (SelectCall s in targetView.selectCalls)
                            {
                                foreach (ParsedTokenSection section in s.selects)
                                {
                                    List<ParsedToken> selectTokens = Utils.CleanupParsedTokenList(section.Tokens, ")");
                                    selectTokens = Utils.CleanupParsedTokenList(selectTokens, "(");
                                    for (int i = 0; i < selectTokens.Count; i++)
                                    {
                                        string lastSelectToken = Utils.GetTokenValue(i-1, selectTokens);
                                        string thisSelectToken = Utils.GetTokenValue(i, selectTokens);
                                        string nextSelectToken = Utils.GetTokenValue(i+1, selectTokens);

                                        if(thisSelectToken.ToLower() == "as" && lastSelectToken.ToLower() != "end")
                                        {
                                            Field f = GetField(lastSelectToken);
                                            if(f != null)
                                            {
                                                if (!aliasFields.ContainsKey(nextSelectToken))
                                                {
                                                    aliasFields.Add(nextSelectToken, f);
                                                }
                                            }
                                        }
                                    }
                ...
                #region Defining Merges
                if (ProcedureName != "" && thisTokenValue.ToLower() == "merge")
                {
                    string fromTarget = nextTokenValue;
                    if (fromTarget == "dbo")
                    {
                ...
                if (ProcedureName != "" && (thisTokenValue.ToLower() == "inner" || thisTokenValue.ToLower() == "outer" || thisTokenValue.ToLower() == "left" || thisTokenValue.ToLower() == "cross"))
                {
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower().Contains("func"))
                {
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower() == "=")
                {
                    string justBefore = lastTokenValue;
                    string justAfter = nextTokenValue;
                    if (justBefore != "" && justAfter != "")
                    {
                        if (justBefore.ToLower() != "time" && justBefore.ToLower() != "real" && justBefore.ToLower() != "money" && justBefore.ToLower()
                            != ")" && justBefore.ToLower() != "!" && justBefore.ToLower() != ">" && justBefore.ToLower() != "<" && justBefore != ") "
                            && justBefore.ToLower().ToLower() != "int" && justBefore.ToLower().ToLower() != "bit" && justBefore.ToLower() != "datetime" && justBefore.ToLower() != "date")
                        {
                            ...
                            justAfter = Utils.MakeSingleSpaces(justAfter);
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower() == "update")
                {
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower() == "insert" && nextTokenValue.ToLower() == "into")
                {
                ...
                if (ProcedureName != "" && thisTokenValue.ToLower() == "exec")
                {
        ...
        public Field GetField(string fieldName)
        {
            ...
            if(fieldName.ToLower() == "date")
            {
                return new Field("date", DataBaseDataType.date, -1, true);
            }
            ...
            //try alias fields
            if (aliasFields.ContainsKey(fieldName))
            {
                return aliasFields[fieldName];
            }


            ErrorManager.Log("could not find field " + fieldName + " in " + ProcedureName);
            return null;
        }
        ...
        public List<Issue> GetIssues()
        {
            ...
            foreach (FunctionCall functionCall in functionCalls)
            {
                        ...
                        for (int i = 0; i < function.Parameters.Count; i++)
                        {
                            ...
                            if (argument != null)
                            {
                                if (argument.definedField)
                                {
                                    ...
                                    {
                                        //not good
                                        issues.Add(new Issue(ProcedureName, Issue.IssueType.FUNC_ARG_IMP, String.Format("Argument {0} ({1}) does not match datatype of parameter {2} ({3}) when calling {4}", argument.FieldName, argument.DataType, parameter.FieldName, parameter.DataType,function.FunctionName)));
                                    }
                                }
                                else
                                {
                ...
                {
                    //ErrorManager.Log("Could not find function: " + functionName + " in " + ProcedureName);
                }
            ...
            foreach (ComparisonCall comparisonCall in comparisonCalls)
            {
                ...
                if (field1 != null && field2 != null)
                {
                    ...
                    {
                        issues.Add(new Issue(ProcedureName, Issue.IssueType.COMPARISON_IMP, String.Format("{0} ({1}) does not match datatype in comparison with {2} ({3}) in {4}", field1.FieldName, field1.DataType, field2.FieldName, field2.DataType, String.Format("{0} = {1}", field1.FieldName, field2.FieldName))));
            ...
            foreach (InsertCall insertCall in insertCalls)
            {
                ...
                {
                    //must be a new table
                    table = DatabaseSimulation.TryGetTable(tableName);
                    if (table != null)
                    {
                        if (!exposedTables.Contains(table))
                        {
                ...
                if (table != null)
                {
                        ...
                        foreach (ParsedTokenSection colToken in columns)
                        {
                            ...
                            if (column != null)
                            {
                                ...
                                if (value != null)
                                {
                                    ...
                                    {
                                        issues.Add(new Issue(ProcedureName, Issue.IssueType.INSERT_IMP, String.Format("{0} ({1}) is inserted into column {2} ({3}) in {4} causing implicit conversion", value.FieldName, value.DataType, column.FieldName, column.DataType,insertCall.TableName)));
                                    }
                                }
                            }
                            else
                    ...
                    {
                        //ErrorManager.Log("Error Insert call has unmatching lengths" + columns.Count + "/" + values.Count + " in: " + ProcedureName);
                    }
                }
                else
                {
                    ErrorManager.Log("Error Insert call has unknown table " + tableName + " in: " + ProcedureName);
            ...
            foreach (ExecCall execCall in execCalls)
            {
                ...
                if (execProcedure != null)
                {
                    ...
                    if (execProcedure.Parameters.Count == execCall.ArgumentSections.Count)
                    {
                        ...
                        foreach (Field parameter in execProcedure.Parameters)
                        {
                            ...
                            if (argField != null)
                            {
                                ...
                                {
                                    issues.Add(new Issue(ProcedureName, Issue.IssueType.SP_ARG_IMP, String.Format("{0} ({1}) argument datatype does not match parameter {2} ({3}) when calling {4}", argField.FieldName, argField.DataType, parameter.FieldName, paramType, execProcedure.ProcedureName)));
                                }

                            }
                            else
                            {
                                ErrorManager.Log("Exec cannot find field: " + execCall.ArgumentSections[i].Tokens[0].Sql + " in: " + ProcedureName);
                            }
                    ...
                    {
                        //ErrorManager.Log("Exec call has wrong argument length" + " in: " + ProcedureName);
                    }
                }
            }
